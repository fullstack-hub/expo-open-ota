...
func TestRequestUploadUrlWithSampleUpdate(t *testing.T) {
	...
	for _, req := range uploadRequests {
		...
		// FetchSelfExpoUsername returns the appId (no Expo account lookup),
		// so the upload-token JWT subject is the app id.
		assert.Equal(t, "test-app-id", sub, "Expected appId sub")
	}
	var (
...
// TestRequestUploadUrlWithValidExpoSession: expo-session credentials were an
// api.expo.dev concept; they are rejected outright (401) — CI must use a
// publish token (OTA_PUBLISH_TOKEN) instead.
func TestRequestUploadUrlWithValidExpoSession(t *testing.T) {
	...
	r.Body = io.NopCloser(bytes.NewReader(uploadRequestsInputJSON))
	handlers.RequestUploadUrlHandler(w, r)
	assert.Equal(t, 401, w.Code, "Expected status code 401: expo-session is not supported")
	assert.Equal(t, "Error validating expo auth\n", w.Body.String(), "Expected error message")
}
